...
public class AuthController {
    ...
    @Operation(summary = "001.회원가입", description = "이메일, 비밀번호, 이름, 전화번호를 입력해 회원을 생성합니다.")
     ...
     */
    @PostMapping("/login")
    @Operation(summary = "002.로그인", description = "이메일과 비밀번호를 확인한 뒤 토큰을 발급합니다.")
     ...
     */
    @PostMapping("/reissue")
    @Operation(summary = "003.토큰 재발급", description = "리프레시 토큰으로 새로운 액세스 토큰과 리프레시 토큰을 발급합니다.")
    public LoginResponse reissue(
        ...
        addRefreshCookie(response, tokenPair);
     ...
     * 쿠키에 리프레시 토큰 저장
     */
    private void addRefreshCookie(HttpServletResponse response, TokenPair tokenPair) {
        ResponseCookie cookie = ResponseCookie.from("refreshToken", tokenPair.getRefreshToken())
                .httpOnly(true)
                .secure(cookieSecure)
                .sameSite("Strict")
                .path("/api/auth")
                .maxAge(Duration.ofSeconds(tokenPair.getRefreshTokenExpiresIn()))
                .build();
